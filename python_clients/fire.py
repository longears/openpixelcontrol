...
def pixel_color(t, coord, ii, n_pixels, random_values, min_coord, max_coord):
    ...
    zp2 = (  zp + color_utils.cos(xp, offset=-t*0.38 + 1.23, period=0.23) * 0.2
                + color_utils.cos(xp, offset= t*0.23 + 2.53, period=0.63) * 0.3  )
    zp3 = (  zp + color_utils.cos(xp, offset=-t*0.42 + 5.62, period=0.27) * 0.2
                + color_utils.cos(xp, offset= t*0.20 + 3.07, period=0.55) * 0.3  )
    zp4 = (  zp + color_utils.cos(xp, offset= t*0.36 + 4.81, period=0.20) * 0.2
                + color_utils.cos(xp, offset=-t*0.26 + 7.94, period=0.67) * 0.3  )

    # make basic vertical gradient
    vgrad = color_utils.cos(color_utils.clamp(zp2, 0, 1), offset=0, period=2, minn=0, maxx=1)
#     vgrad = 1 - color_utils.clamp(zp, 0, 1)

    # small fast noise
    noise1 = (   color_utils.cos(xp,  offset=-3 * t/4, period=0.3)
               + color_utils.cos(yp,  offset= 3 * t/4, period=0.3)
               + color_utils.cos(zp3, offset= 3 * t,   period=0.3)  ) / 3

    # big slow noise
    noise2 = (   color_utils.cos(xp,  offset=-0.9 * t/2, period=0.8)
               + color_utils.cos(yp,  offset= 0.9 * t/2, period=0.8)
               + color_utils.cos(zp4, offset= 0.9 * t,   period=0.8)  ) / 3

    # combine vgradient with noise
    v = (   vgrad 
          + color_utils.remap(noise1, 0,1, -1,1)*0.2 
          + color_utils.remap(noise2, 0,1, -1,1)*0.5  )
...
#     v = color_utils.cos( color_utils.clamp(v,0,1), offset=0, period=2, minn=1, maxx=0 )
    ...
    r,g,b = color_utils.contrast((r,g,b), 0.5, 1.2)

#     r,g,b = color_utils.clip_black_by_luminance((r,g,b), 0.3)
